...
interface IntakePreInjuryTreatmentsProps {
  treatments: TreatmentEntry[];
  onChange: (treatments: TreatmentEntry[]) => void;
}

export function IntakePreInjuryTreatments({
  treatments,
  onChange,
}: IntakePreInjuryTreatmentsProps) {
  const addTreatment = () => {
      ...
      <CardContent className="p-6">
        ...
        <div className="space-y-4">
          {treatments.map((treatment, index) => (
            <div key={treatment.id} className="p-4 border rounded-lg space-y-3 bg-card relative">
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
                <div>
                  <Label htmlFor={`pre-treatment-provider-${index}`}>Provider/Facility</Label>
                  <Input
                    id={`pre-treatment-provider-${index}`}
                    value={treatment.provider}
                    onChange={(e) => updateTreatment(index, 'provider', e.target.value)}
                    placeholder="e.g., Dr. Smith PT Clinic"
                  />
                </div>
                <div>
                  <Label htmlFor={`pre-treatment-type-${index}`}>Type of Treatment</Label>
                  <Input
                    id={`pre-treatment-type-${index}`}
                ...
                <div>
                  <Label htmlFor={`pre-treatment-frequency-${index}`}>Frequency</Label>
                  <Input
                    id={`pre-treatment-frequency-${index}`}
                ...
                <div>
                  <Label htmlFor={`pre-treatment-start-${index}`}>Start Date</Label>
                  <Input
                    id={`pre-treatment-start-${index}`}
                    type="date"
          ...
          >
            ...
            Add Pre-Injury Treatment/Service
          </Button>
        </div>
      </CardContent>
    </Card>
